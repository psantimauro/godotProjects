...
func _ready() -> void:
	...
	var res = BuildingManager.get_resource_from_building_type(type)
...
func _on_job_unlocked(job, building_type):
	if building_type == type: ## maybe check iif we have the type? 
		unlocked_jobs.append(job)
		work_ui.add_job(job)

func _on_tech_unlocked(tech, building_type):
